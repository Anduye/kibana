...
export interface DashboardContainerInput<E extends EmbeddableInput = EmbeddableInput> extends ContainerInput {
  viewMode: ViewMode;
  filters: Filter[];
  query: Query;
  timeRange: TimeRange;
...
> {
  ...
  public createNewPanelState<EEI extends EmbeddableInput = EmbeddableInput>(
    { type, initialInput }:
    { type: string, initialInput: Partial<EEI> }): DashboardPanelState<EEI> {
    const embeddableId = initialInput.id || uuid.v4();
    return createPanelState<EEI>({ ...initialInput, id: embeddableId }, type, Object.values(this.input.panels));
  }
  ...
  public getInputForEmbeddable<DashboardEmbeddableInput>(
    panelState: DashboardPanelState): DashboardEmbeddableInput {
